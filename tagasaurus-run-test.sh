...
if [[ -z $1 ]]; then ts_path_input=$(dirname "$0"); else 
  if [[ -d $1 ]]; then ts_path_input="$1"; else echo "Input path is \"$1\" and doesn't exist. Exit."; set +x; return; fi 
fi
...
remount_fat () {
  ...
  if [[ "$EUID" = 0 ]]; then
    echo "Please use 'sudo' instead 'root'. Exit." ; set +x; return;
  else
    sudo -k
    echo "Please enter \"$(whoami)\" password."
    if sudo true; then echo "Password correct. Remounting."
    else echo "Wrong password for \"$(whoami)\" or 'sudo' not configured. Remount required a 'sudo'. Exit."; set +x; return; fi
  fi
  cd $HOME;
  sudo umount -l "$1"
  sudo mkdir -p "$3"
  sudo mount -o rw,uid=$(id -u),gid=$(id -g),utf8 "$2" "$3"
  (($? != 0)) && { echo "Remount Error. Quit."; set +x; return; }
}
...
if [[ -f ./tagasaurus && "application" == $(file -b --mime-type ./tagasaurus | sed 's|/.*||') ]]; then 
  ...
  if [[ -n $noexec_mnt ]]; then
    ...
    if ! ts_found=$(find "$ts_path_input" -maxdepth 2 -type f -iname "tagasaurus"); then echo "Searching error. Exit."; set +x; return; fi
    parent=${ts_found%/*}
...
# Else serching all Tagasaurus application folders
  ...
  if ! ts_found=$(find "$ts_path_input" -maxdepth 2 -type f -iname "tagasaurus"); then echo "Searching error. Exit."; set +x; return; fi  
  if [[ -z "$ts_found" ]]; then echo "Tagasaurus not found. Exit"; set +x; return; fi
  ...
  if [[ -z "$ts_path_checked" ]]; then echo "Tagasaurus not found."; set +x; return;fi
